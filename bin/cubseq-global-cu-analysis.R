...
"cubseq-global-cu-analysis.R

Usage:
    cubseq-global-cu-analysis.R [--fasta-dir=<fasta_dir>] [--tax-id=<tax_id>]
    
Options:
    --fasta-dir=<fasta_dir>                 Directory containing fasta files for mutated transcriptomes [default: .].
...
arguments <- docopt(doc, version = "cubseq-global-cu-analysis.R")
...
#   df$abs.diff.HEG_kazusa <- abs(df$kazusa.rf - df$protein.rf)
#   df$abs.diff.LEG_kazusa <- abs(df$kazusa.rf - df$leg.rf)
#   df$abs.diff.HEG_LEG <- abs(df$protein.rf - df$leg.rf)
  
#   # calculate pairwise difference (squared)
#   df$sq.diff.HEG_kazusa <- (df$kazusa.rf - df$protein.rf)^2
#   df$sq.diff.LEG_kazusa <- (df$kazusa.rf - df$leg.rf)^2
#   df$sq.diff.HEG_LEG <- (df$protein.rf - df$leg.rf)^2
...
main <- function() {
    ...
    # count codons for HEG and LEG
    protein.cc <- get_cubseq_codon_counts(arguments$fasta_dir)
    ...
    all.rf <- merge(protein.rf, kazusa.rf) %>%
        merge(cocoputs.rf) %>%
        select(., aa, codon, protein.rf, kazusa.rf, cocoputs.rf)

    # # calculate absolute and squared difference
    # all.rf <- calculate_difference(all.rf)

    # calculate log2 comparison
    all.rf$log2.heg.kazusa <- log2(all.rf$protein.rf / all.rf$kazusa.rf)
    all.rf$log2.leg.kazusa <- log2(all.rf$leg.rf / all.rf$kazusa.rf)
    all.rf$log2.heg.leg <- log2(all.rf$protein.rf / all.rf$leg.rf)
    all.rf$log2.heg.cocoputs <- log2(all.rf$protein.rf / all.rf$cocoputs.rf)
